...
as_ml_vector_serialize(const as_vector* vector, as_ml_vector_element_type element_type, as_bytes* bytes)
{
	...
	*(uint32_t*)p = cf_swap_to_be32(AS_ML_VECTOR_MAGIC_NUMBER);
	p += 4;

	// Write version (big-endian)
	*(uint32_t*)p = cf_swap_to_be32(AS_ML_VECTOR_VERSION);
	p += 4;

	// Write element count (big-endian)
	*(uint32_t*)p = cf_swap_to_be32(vector->size);
	p += 4;

	// Write element type (big-endian)
	*(uint32_t*)p = cf_swap_to_be32((uint32_t)element_type);
	p += 4;

	// Write vector data (native byte order as specified)
...
as_ml_vector_deserialize(const as_bytes* bytes, as_vector* vector, as_ml_vector_element_type* element_type)
{
	...
	uint32_t magic = cf_swap_from_be32(*(uint32_t*)p);
	...
	uint32_t version = cf_swap_from_be32(*(uint32_t*)p);
	...
	// Read element count
	uint32_t element_count = cf_swap_from_be32(*(uint32_t*)p);
	p += 4;

	// Read element type
	uint32_t element_type_raw = cf_swap_from_be32(*(uint32_t*)p);
	p += 4;
